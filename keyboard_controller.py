...
class KeyMapping(object):
	...
	CenterTrajectory = QtCore.Qt.Key.Key_V
	LinearTrajectory = QtCore.Qt.Key.Key_N
	CircleTrajectory = QtCore.Qt.Key.Key_M
	
	NTrajectory = QtCore.Qt.Key.Key_B
	RaceTrajectory = QtCore.Qt.Key.Key_C
	DefaultHeight = QtCore.Qt.Key.Key_I
	EmergencyHeight = QtCore.Qt.Key.Key_O
	...
	def keyPressEvent(self, event):
		...
		if controller is not None and not event.isAutoRepeat():
			...
			if key == KeyMapping.ThrottleCut:
				...
				controller.SendLand()
			#---------------------------------------------------------------------
			# Adding the commands for various additional inputs being pressed.
			#---------------------------------------------------------------------
			elif key == KeyMapping.CenterTrajectory:
				controller.SendCenter()
			elif key == KeyMapping.LinearTrajectory:
				controller.SendLinear()
			elif key == KeyMapping.CircleTrajectory:
				controller.SendCircle()
			elif key == KeyMapping.NTrajectory:
				controller.SendNTrajectory()
			elif key == KeyMapping.RaceTrajectory:
				controller.SendRaceTrajectory()
			elif key == KeyMapping.DefaultHeight:
				controller.SendDefaultHeight()
			elif key == KeyMapping.EmergencyHeight:
				controller.SendEmergencyHeight()
			#---------------------------------------------------------------------
